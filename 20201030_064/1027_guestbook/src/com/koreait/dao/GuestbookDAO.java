...
//	DeleteService 클래스에서 mapper와 삭제할 글번호를 넘겨받고 테이블에 저장된 글 1건을 삭제하는 guestbook.xml 파일의 delete sql 명령을 실행하는
//	메소드
	public void delete(SqlMapClient mapper, int idx) throws SQLException {
		System.out.println("GuestbookDAO 클래스 delete() 메소드");
		mapper.delete("delete", idx);
	}
	
//	UpdateService 클래스에서 mapper와 수정할 정보가 저장된 객체를 넘겨받고 테이블에 저장된 글 1건을 수정하는 guestbook.xml 파일의 update sql 
//	명령을 실행하는 메소드
	public void update(SqlMapClient mapper, GuestbookVO vo) throws SQLException {
		System.out.println("GuestbookDAO 클래스 update() 메소드");
