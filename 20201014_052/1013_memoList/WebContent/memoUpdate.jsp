...
//			수정할 글번호에 해당되는 글이 테이블에 존재하는가 확인하기 위해 수정할 글번호에 해당되는 글 1건을 얻어온다.
...
<form action="updateOK.jsp" method="post">
	<table width="850" align="center" border="1" cellpadding="5" cellspacing="0">
		...
		<tr>
			...
			<td>
				...
				<input type="text" name="memo" value="<%=rs.getString("memo")%>" size="98"/>
			</td>
		</tr>
		<tr>
			<td>작성일</td>
	...
	<!-- 수정할 글번호와 수정 작업 후 돌아갈 페이지 번호도 deleteOK.jsp로 넘겨줘야 한다. -->
...
//			수정할 글번호와 수정 후 돌아갈 페이지 번호가 모두 숫자가 아니므로 에러 메시지를 출력하고 memoList4.jsp로 돌려보낸다.
			out.println("<script>");
			out.println("alert('수정할 글번호 또는 수정 후 돌아갈 페이지 번호가 모두 숫자가 아닙니다.')");
			out.println("location.href='memoList4.jsp'");
			out.println("</script>");
		}
	} else {
//		수정할 글번호와 수정 후 돌아갈 페이지 번호가 모두 넘어오지 않았으므로 에러 메시지를 출력하고 memoList4.jsp로 돌려보낸다.
		out.println("<script>");
		out.println("alert('수정할 글번호 또는 수정 후 돌아갈 페이지 번호가 넘어오지 않았습니다.')");
		out.println("location.href='memoList4.jsp'");
		out.println("</script>");
	}
%>
