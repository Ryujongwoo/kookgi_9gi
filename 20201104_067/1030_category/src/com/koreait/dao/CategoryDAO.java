...
//	CategoryService 클래스에서 mapper와 삭제한 카테고리 그룹을 넘겨받고 테이블에서 삭제한 카테고리 그룹에 해당되는 전체 카테고리 목록을 얻어오는
...
//	CategoryService 클래스에서 mapper와 seq를 수정할 카테고리 번호와 seq를 수정할 데이터가 저장된 HashMap 객체를 넘겨받고 테이블에서 seq를
//	수정하는 category.xml 파일의 update sql 명령을 실행하는 메소드
