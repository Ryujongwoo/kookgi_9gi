...
//	insert sql 명령을 실행하는 메소드를 호출하는 메소드를 호출하는 메소드
...
//	얻어오는 select sql 명령을 실행하는 메소드를 호출하는 메소드를 호출하는 메소드
...
//	update.jsp에서 호출되는 수정할 카테고리 정보가 저장된 객체를 넘겨받고 mapper를 얻어온 후 DAO 클래스의 테이블에 저장된 카테고리 한 건을 
...
//	delete.jsp에서 호출되는 삭제할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 DAO 클래스의 테이블에 저장된 카테고리 한 건을 "삭제된 카테고리
//	입니다."로 수정하는 update sql 명령을 실행하는 메소드를 호출하는 메소드
...
//	delete.jsp에서 호출되는 삭제할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 DAO 클래스의 테이블에 저장된 카테고리 한 건의 deleteCheck 필드를
//	"yes"로 수정하는 update sql 명령을 실행하는 메소드를 호출하는 메소드
...
//	deleteRestore.jsp에서 호출되는 복구할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 DAO 클래스의 테이블에 저장된 카테고리 한 건의 deleteCheck 
//	필드를 "no"로 수정하는 update sql 명령을 실행하는 메소드를 호출하는 메소드
...
//	deleteReport.jsp에서 호출되는 신고할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 DAO 클래스의 테이블에 저장된 카테고리 한 건의 deleteReport 
//	필드를 1증가 시키는 update sql 명령을 실행하는 메소드를 호출하는 메소드
...
//	delete.jsp에서 호출되는 삭제할 카테고리 정보가 저장된 객체를 넘겨받고 mapper를 얻어온 후 DAO 클래스의 테이블에 저장된 삭제할 카테고리
...
//	delete.jsp에서 호출되는 seq를 다시 부여할 카테고리 그룹(ref)를 넘겨받고 mapper를 얻어온 후 DAO 클래스의 테이블에 저장된 삭제한 카테고리
//	그룹의 seq를 0부터 다시 부여하는 update sql 명령을 실행하는 메소드를 호출하는 메소드
	public void reSeq(int ref) {
		System.out.println("CategoryService 클래스의 reSeq() 메소드");
		SqlSession mapper = MySession.getSession();
		CategoryDAO dao = CategoryDAO.getInstance();
		
//		seq를 다시 부여할 카테고리 그룹만 얻어와 ArrayList에 저장한다.
		ArrayList<CategoryVO> refList = dao.selectRef(mapper, ref);
//		for (CategoryVO vo : refList) {
//			System.out.println(vo);
//		}
		
//		ArrayList로 얻어온 카테고리 그룹에 카테고리 개수 만큼 반복하며 seq를 0부터 다시 부여한다.
