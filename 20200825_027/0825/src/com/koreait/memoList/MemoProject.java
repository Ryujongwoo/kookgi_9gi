...
//	키보드로 텍스트 파일의 이름을 입력받아 텍스트 파일의 데이터를 MemoList 클래스의 memoList ArrayList에 저장한다.
...
//		텍스트 파일의 데이터를 읽어 MemoList 클래스의 memoList ArrayList에 저장하는 메소드를 실행한다.
		memoList.fileRead(filename);
		
		System.out.println(filename + ".txt 파일에서 읽기 완료!!!");
	}

//	키보드로 텍스트 파일의 이름을 입력받아 MemoList 클래스의 memoList ArrayList에 저장된 데이터를 텍스트 파일로 출력한다.
...
//		MemoList 클래스의 memoList ArrayList에 저장된 데이터를 텍스트 파일로 출력하는 메소드를 실행한다.
		memoList.fileWrite(filename);
		
		System.out.println(filename + ".txt 파일에 쓰기 완료!!!");
	}

//	키보드로 수정할 글번호를 입력받아 수정할 글 1건을 화면에 표시하고 비밀번호를 입력받아 memoList ArrayList 저장된 글 1건을 수정한다.
...
//			글을 수정하기 위해 입력한 비밀번호(password)와 수정할 글의 비밀번호(original.getPassword())가 일치하면 글을 수정하고 일치하지
//			않으면 비밀번호가 일치하지 않다는 메시지를 출력한다.
...
//	키보드로 삭제할 글번호를 입력받아 삭제할 글 1건을 화면에 표시하고 비밀번호를 입력받아 memoList ArrayList 저장된 글 1건을 삭제한다.
...
//			글을 삭제하기 위해 입력한 비밀번호(password)와 삭제할 글의 비밀번호(original.getPassword())가 일치하면 글을 삭제하고 일치하지
//			않으면 비밀번호가 일치하지 않다는 메시지를 출력한다.
			if(password.equals(original.getPassword())) {
